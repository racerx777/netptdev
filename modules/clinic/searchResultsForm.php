...
<form method="post" name="searchResults">
	<table border="1" cellpadding="3" cellspacing="0" width="100%">
		...
		while($row = mysqli_fetch_array($result,MYSQLI_ASSOC)) {
			if($row['cminactive'] == '1') {
				if($row['pausestate'] == '1'){
					$rowstyle=' style="background-color:#fceb92;"';	
					$btnVal = 'UnPause';
				}else{
					$rowstyle=' style="background-color:#ff9ba3;"'; 
					$btnVal = 'Pause';
					$disabled = 'disabled';
				}
				$togglebutton = '<input name="button['.$row["cmcnum"].']" title=\'This clinic is currently set to "Inactive"\' type="submit" value="Make Active">';
			}
			else {
				if($row['pausestate'] == '1'){
					$rowstyle=' style="background-color:#fceb92;"';	
				...
				$togglebutton = '<input name="button['.$row["cmcnum"].']" type="submit" value="Make Inactive">';
...
?>
		<tr<?php echo $rowstyle; ?>>
			...
			<td><input name="button[<?php echo $row["cmcnum"];?>]" type="submit" value="Edit" />
				<input name="button[<?php echo $row["cmcnum"];?>]" data-id="<?php echo $row["cmid"]; ?>" <?php if($btnVal=='Pause'){echo "title='Paused Clinics will not display for scheduling.'";} ?>class="pause-btn" type="button" value="<?php echo $btnVal; ?>" <?php echo $disabled; ?> />
				<?php echo $togglebutton; ?></td>
		</tr>
		<?php
		}
?>
...
<script>
	...
	e.preventDefault();
	var pause=$(this).data('id');
	var value=$(this).val();
		$.ajax({
			type: 'post',
			url: 'modules/clinic/pauseajax.php',
			data: {pause:pause,value:value},
