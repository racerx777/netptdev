	...
	$html .= '<table style="text-align:center;border-collapse: collapse; border: solid;table-layout:fixed;" width="100%" >
		...
		while($row=mysqli_fetch_assoc($result)) {
			...
			if($displaydetailtable) {
				...
				$html .='<tr style="border-collapse:collapse; border:solid; border-color:#CCCCCC;"  >
					<td align="center" style="font-size:8px;width:5%" >'.$clinicname.'</td>
					...
					if(isset($cancelreason)) {
						$html .= '<td   align="left" style="font-size:10px;width:30%;border:0;" nowrap="nowrap">'.$cancelreason.'</td>';
						$html .= '<td></td>';
						$html .= '<td></td>';
						$html .= '<td></td>';
					}else{
					...
					$html .= '<td align="center" style="font-size:8px;width:4%" >'.$patient.'</td>
...
if($displaysummarytable) {
		//echo mysqli_num_rows($result);
	  	//$c = mysqli_num_rows($result) + 10;
		if(mysqli_num_rows($result) % 30 >= 25 && $displaydetailtable) {
		...
		<table cellpadding="0" cellspacing="0" border="1" width="720px">
			...
			<tr>
				<td><table style="text-align:center; border-collapse: collapse; border: solid;" cellpadding="3" width="100%">
						<tr style=" border-color:#CCCCCC; border:solid;">
					...
					foreach($bus as $bucode=>$val) { 
						...
						$html .= '<tr style=" border-color:#CCCCCC; border:solid;">
			...
			$html .= '<tr style="border-collapse:collapse; border:solid;">
				<td><table style="text-align:center; border-collapse: collapse; border: solid;  border-color:#CCCCCC;" cellpadding="3" width="100%">
						<tr>
							<th  valign="bottom" align="left">Schedulers Summary</th>
							<th  valign="bottom" bgcolor="'.$bgcolor['report'].'">OTH</th>
			...
			foreach($schedulers as $scheduler=>$value) { 
				...
				if(is_array($schs["$scheduler"])) {
					...
					$schsavg="";
						...
						$html .= '<tr style="border-bottom-color:#FFFFFF; border-color:#CCCCCC; border:solid;">
