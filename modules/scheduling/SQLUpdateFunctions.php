...
//dump("cshid",$cshid);
	...
	switch($button):
		...
		case 'Confirm Schedule Referral':
			...
			if(!empty($_POST['crfvisitdate']) && isset($_POST['crfvisitdate'])){
				$_SESSION['newdate'] = $appointmentdate; 
			}
			if(!empty($_POST['crcnum']) && isset($_POST['crcnum'])){
				$_SESSION['clinic'] = $_POST['crcnum']; 
			}
			// if(!empty($_POST['crfvisitdate'])){
			// 	$apptscheduleddate=date("Y-m-d H:i:s", time());
			// 	$query1 = "UPDATE cases SET crapptdate='$appointmentdate', crapptscheduleddate='$apptscheduleddate' WHERE crid='$csqid'";
...
function caseschedule($csqid, $newstatus, $clinic, $appointment) {
	if(isset($_SESSION['newdate']) && !empty($_SESSION['newdate'])){
		$appointment = $_SESSION['newdate'];
	}
	...
	if(empty($clinic)){
		...
		require_once($_SERVER['DOCUMENT_ROOT'] . '/config/mysql/wsptn_db.php');
		$dbhandle = dbconnect();
		$csnewqquery2 = "SELECT crcnum  FROM `cases` WHERE `crid` = '$csqid'";
		if($csqnewresult2 = mysqli_query($dbhandle,$csnewqquery2)) { 
			if($csqnewrow2 = mysqli_fetch_assoc($csqnewresult2)) {
				$clinic = $csqnewrow2['crcnum'];
			}
		}
	}
