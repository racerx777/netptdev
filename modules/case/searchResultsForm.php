		...
		<tr <?php echo $rowstyle; ?>>
			...
			<td><?php
				if(!empty($row["crapptdate"]) && $row["crcasestatuscode"]!='CAN')
					...
					echo displayDate($row["crapptdate"]).' '.'<a href="#"><img class="edit-ic" src="/img/edit-ic.png" style="height:15px;"/></a>'.''.displayTime($row["crapptdate"]);
				?>&nbsp;
			</td>
			<td><?php
			...
			<td><input name="button[<?php echo $row["crid"]?>]" class="edit-case-btn" type="submit" value="Edit Case" />
...
$(document).ready(function(){
	$(document).on('click','.edit-ic',function(e){
		e.preventDefault();
		$(this).closest('tr').find('.edit-case-btn').trigger('click');
	}); 
});
</script>