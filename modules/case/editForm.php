...
//	
	$query = "
		SELECT $fieldslist, crcanceldate, crptosstatus, paaddress1, pacity, pastate, pazip, crcasestatuscode, crpaid, paid, palname, pafname
		FROM $table 
		LEFT JOIN patients
			on crpaid=paid
		WHERE $keyfield='$crid'
...
$test = mysqli_query($dbhandle,$query);
while($row = mysqli_fetch_array($test))
	{
		$address = $row['paaddress1'].' '.$row['pacity'].' '.$row['pastate'].' '.$row['pazip'];
		$address = str_replace(' ', '+', $address);
		$GLOBALS['a'] =  $address;
		$_SESSION['a'] =  $address;
		$p_lat = $row['p_latitude'];
		$p_lng = $row['p_longitude'];
		if(empty($p_lat) || empty($p_lng)){
			$address_new = $row['paaddress1'].' '.$row['pacity'].' '.$row['pastate'].' '.$row['pazip'];
			...
			$update_new="UPDATE patients SET p_latitude='".$lat_new."', p_longitude='".$lng_new."' WHERE paid='".$row['paid']."'";
			mysqli_query($dbhandle,$update_new);
			$_SESSION['plat'] =  $lat_new;
			$_SESSION['plong'] = $lng_new;
		}else{
			$_SESSION['plat'] =  $p_lat;
			$_SESSION['plong'] = $p_lng;
		}
...
// if (isset($_GET['test'])) {
	$google_option_val = '';
	// $googleoptquery = "SELECT * FROM `case_google_option` WHERE `case_id` = '$crid'";
	...
	// 		$google_option_val = urldecode($googleoptquery_att_row['google_option']);
	// 	}
	// }else{
		
		// $sel_val = getSelectOptions($arrayofarrayitems=getClinicTypeOptions($_POST['crtherapytypecode']), $optionvaluefield='value', $arrayofoptionfields=array('title'=>' (', 'value'=>')', ' ', 'distance'=>' '), $defaultoption=$_POST['crcnum'], $addblankoption=FALSE, $arraykey='sel_val', $arrayofmatchvalues=array());
		// if(!empty($sel_val) && $crid != 0){
		// 	$sel_val =urlencode($sel_val);
		// 	$google_option_query = "INSERT INTO case_google_option (case_id, google_option) VALUES ('$crid', '$sel_val')";
		// 	mysqli_query($dbhandle,$google_option_query);
		// 	$googleoptquery1 = "SELECT * FROM `case_google_option` WHERE `case_id` = '$crid'";
		// 	$googleoptquery1_connect = mysqli_query($dbhandle,$googleoptquery1);
		// 	if (mysqli_num_rows($googleoptquery1_connect) > 0) {
		// 		while($googleoptquery1_att_row = mysqli_fetch_array($googleoptquery1_connect)){
		// 			$google_option_val = urldecode($googleoptquery1_att_row['google_option']);
		// 		}
		// 	}

		// }else{
			if(empty($_SESSION['a'])){
				$paidquery = "SELECT * FROM `patients` WHERE `paid` = '".$_POST['paid']."'";
				$paidquery_connect = mysqli_query($dbhandle,$paidquery);
				if (mysqli_num_rows($paidquery_connect) > 0) {
					while($paidquery_att_row = mysqli_fetch_array($paidquery_connect)){
						$address = $paidquery_att_row['paaddress1'].' '.$paidquery_att_row['pacity'].' '.$paidquery_att_row['pastate'].' '.$paidquery_att_row['pazip'];
						$address = str_replace(' ', '+', $address);
						$GLOBALS['a'] =  $address;
						$_SESSION['a'] =  $address;
						$_SESSION['plat'] =  $paidquery_att_row['p_latitude'];
						$_SESSION['plong'] = $paidquery_att_row['p_longitude'];
					}
				}
			}
			$google_option_val = getSelectOptions($arrayofarrayitems=getClinicTypeOptions($_POST['crtherapytypecode']), $optionvaluefield='value', $arrayofoptionfields=array('title'=>' (', 'value'=>')', ' ', 'distance'=>' '), $defaultoption=$_POST['crcnum'], $addblankoption=FALSE, $arraykey='', $arrayofmatchvalues=array());
		// }
	// }
// }
?>
...
<div class="centerFieldset" style="margin-top:100px;">
	<form action="<?=$_SERVER['PHP_SELF'];?>" method="post" name="editForm">
			...
			<tr>
				...
				<td><select name="crcnum" id="crcnum">
					<option>Select</option>
						<?php
						// $clinic_option_val= '';
						// if($_SESSION['paid_'.$_POST['crcnum']] != $_POST['crcnum']){
						// 	unset($_SESSION['paid_'.$_POST['paid']]);
						// 	unset($_SESSION['paid_'.$_POST['crcnum']]);
						// }
						// if(!isset($_SESSION['paid_'.$_POST['paid']]) && empty($_SESSION['paid_'.$_POST['paid']])){
						// 	$_SESSION['paid_'.$_POST['paid']] = getSelectOptions($arrayofarrayitems=getClinicTypeOptions($_POST['crtherapytypecode']), $optionvaluefield='value', $arrayofoptionfields=array('title'=>' (', 'value'=>')', ' ', 'distance'=>' '), $defaultoption=$_POST['crcnum'], $addblankoption=FALSE, $arraykey='', $arrayofmatchvalues=array());
						// 	$clinic_option_val = $_SESSION['paid_'.$_POST['paid']];
						// 	$_SESSION['paid_'.$_POST['crcnum']] = $_POST['crcnum'];
						// }else{
						// 	$clinic_option_val = $_SESSION['paid_'.$_POST['paid']];
						// }
						?>
						<?php echo $google_option_val; ?>

					</select>
					<input type="hidden" class="crtherapytypecode-val" value="<?php echo $_POST['crtherapytypecode']; ?>">
					<input type="hidden" class="crcnum-val" value="<?php echo $_POST['crcnum']; ?>">
					<input type="hidden" class="crid-val" value="<?php echo $crid; ?>">
					<!-- <a class="load-google-api" style="background-color: #EBEBED; padding: 2px 15px; border-radius: 4px; border: 1px solid gray; cursor: pointer;">Find Closest Clinic</a> -->
			...
			<form class="add-form">
				...
				<table>
					<tbody>
						...
						<tr>
							<td>Attorney Address 2</td>
							<td><input name="address2" class="add-address2" type="text" value=""></td>
						</tr>
						<tr>
							<td>Attorney City </td>
			...
			<form  class="edit-form">
				...
				<table>
					<tbody>
						...
						<tr>
							<td>Attorney Address 2</td>
							<td><input name="address2" class="edit-address2" type="text" value=""></td>
						</tr>
						<tr>
							<td>Attorney City </td>
...
<style type="text/css" media="screen">
	...
	<script>
		$(document).ready(function(){
			...
			$('.load-google-api').on('click',function(e){
				e.preventDefault();
				$("#loader").show();
				var crtherapytypecode = $(".crtherapytypecode-val").val();
				var crcnum = $(".crcnum-val").val();
				$.ajax({
					type: 'post',
					url: 'modules/case/ajax/google-option-ajax.php',
					dataType: 'json',
					data: {'crtherapytypecode': crtherapytypecode,'crcnum': crcnum,'check': 'option_load'},
					success: function (data) {
						$("#loader").hide();
						$("#crcnum").html(data);
					}
			...
			$(document).on('change', '#crcnum', function(e) {
				var crid = $(".crid-val").val();
				e.preventDefault();
				$.ajax({
					type: 'post',
					url: 'modules/case/ajax/google-option-ajax.php',
					dataType: 'json',
					data: {'check': 'option_delete','crid': crid},
					success: function (data) {
					}
				});
			});
