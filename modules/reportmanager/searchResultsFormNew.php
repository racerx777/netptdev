...
//	$where[]="crcnum in ($useraccessclinics)";

//$where = "WHERE (
// DAYOFYEAR(crapptdate) between
//  DAYOFYEAR( DATE_SUB(CURRENT_DATE(), INTERVAL 2 DAY) )
//  and
//  DAYOFYEAR( CURRENT_DATE() )
// or
// DAYOFYEAR(rhdate) between
//  DAYOFYEAR( DATE_SUB(CURRENT_DATE(), INTERVAL 2 DAY) )
//  and
//  DAYOFYEAR( CURRENT_DATE() )
// )
// and crcnum='".$default['crcnum']."'";

// Add logic to select cases that have an initial evaluation billed and no initial evaluation report
// 98774 or 97001 = New Patient Evaluation NPE & 99081 = acupuncture supplemental report
// 98774 or 97002 = re-admit with report
// 98776 or 97002 = re-eval with report
unset($billednotreported);
$selectbillednotreported="
SELECT distinct tbdthpnum
FROM treatment_billing_detail
LEFT JOIN report_header
ON tbdthpnum=rhpnum
WHERE rhid IS NULL and
tbdthid>284200 and tbdcode in ('98774','98776','97001','97002','99081')
and tbdthcnum='".$default['crcnum']."'
";

$selectbillednotreported="
SELECT distinct tbdthpnum
FROM treatment_billing_detail
LEFT JOIN report_header ON tbdthpnum=rhpnum
WHERE rhid IS NULL and tbdthid>395000 and tbdcode in ('98774','98776','97001','97002','99081')
and tbdthcnum IN (".$useraccessclinics.")
";
//dump("selectbillednotreported",$selectbillednotreported);
$wherebillednotreported="";
if($resultbillednotreported=mysqli_query($dbhandle,$selectbillednotreported)) {
	while($rowbillednotreported=mysqli_fetch_assoc($resultbillednotreported))
		$billednotreported[]=$rowbillednotreported['tbdthpnum'];
	if(is_array($billednotreported) && count($billednotreported) >0 )
		$wherebillednotreported="crpnum IN ('".implode("', '", $billednotreported)."')";
}

unset($unfinished);
$selectunfinished="SELECT rhcrid FROM report_header WHERE rhcnum='".$default['crcnum']."' AND rhcrid<>0 AND rhstatus!='FILED'";
...
if(is_array($unfinished) && count($unfinished) >0 )
	$whereunfinished=" OR ( crid IN ('".implode("', '", $unfinished)."'))";

$where = "WHERE
(
YEAR(crapptdate) = YEAR(CURRENT_DATE()) AND
DAYOFYEAR(crapptdate) BETWEEN DAYOFYEAR( DATE_SUB(CURRENT_DATE(), INTERVAL 2 DAY) ) AND DAYOFYEAR( CURRENT_DATE() )
AND crcnum='".$default['crcnum']."'
)
$whereunfinished
$wherebillednotreported
...
//$query  = "
//	SELECT
//		crid, crpnum, crinjurydate, crcnum, crapptdate, crcasestatuscode, crtherapytypecode, crcasetypecode, cricd9code1, cricd9code2, cricd9code3, cricd9code4, crdxbodydescriptor, crdxbodypart,
//		paid, palname, pafname, padob, paphone1, passn,
//		cmbnum, cmpgmcode
//	FROM cases
//	LEFT JOIN report_header ON crid=rhcrid
//	LEFT JOIN patients
//	ON crpaid=paid
//	LEFT JOIN master_clinics
//	ON crcnum=cmcnum
//	$where
//	ORDER BY rhdate, crapptdate
//	LIMIT 100
//";
$query  = "
	SELECT
...
// list recent patients and patients that were billed a report without a report yet
if( !empty($wherebillednotreported) )
	$criteria="$wherebillednotreported";
//	$criteria="(DATEDIFF(CURDATE(), lvisit)<=4 OR $wherebillednotreported)";
else
	$criteria="DATEDIFF(CURDATE(), lvisit)<=4";

$wheresql = "WHERE crcnum IN ($useraccessclinics) AND $criteria";
$query  = "
...
<div class="containedBox">
	<fieldset>
	<legend style="font-size:large;">New and Recent Patient Report List:</legend>
