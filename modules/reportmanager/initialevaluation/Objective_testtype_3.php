...
$options['rdbtrtmname']=getSelectOptions($arrayofarrayitems=$valid_girth_units, $optionvaluefield='value', $arrayofoptionfields=array('value'=>''), $defaultoption=NULL, $addblankoption=FALSE, $arraykey=NULL, $arrayofmatchvalues=array());
...
// Current Values from database
		...
		$cell[]=getDropDownHTML($tableid, $rowid, "rdbtrtname", $rdbtid, "valid_tests_".$bp."_".$gp, 'report[detail_bodypart_test]['.$rdbtid.'][rdbtrtname]', $array['rdbtrtname'], 30, 64 , $options['rdbtrtname']);

		$cell[]=getDropDownHTML($tableid, $rowid, "rdbtresult1[right]", $rdbtid, NULL, 'report[detail_bodypart_test]['.$rdbtid.'][rdbtresult1][right]', $array['rdbtresult1']['right'], 8, 20, NULL);

		if($bilateral) 
			$cell[]=getDropDownHTML($tableid, $rowid, "rdbtresult1[left]", $rdbtid, NULL, 'report[detail_bodypart_test]['.$rdbtid.'][rdbtresult1][left]', $array['rdbtresult1']['left'], 8, 20, NULL);

// Comparison Values from database
		if($requirecompreportdate) {
			$cell[]=getDropDownHTML($tableid, $rowid, "rdbtresult2[right]", $rdbtid, NULL, 'report[detail_bodypart_test]['.$rdbtid.'][rdbtresult2][right]', $array['rdbtresult2']['right'], 8, 20, NULL);
		
			if($bilateral) {
				$cell[]=getDropDownHTML($tableid, $rowid, "rdbtresult2[left]", $rdbtid, NULL, 'report[detail_bodypart_test]['.$rdbtid.'][rdbtresult2][left]', $array['rdbtresult2']['left'], 8, 20, NULL);
			}
		}

		$cell[]=getDropDownHTML($tableid, $rowid, "rdbtrtmname", $rdbtid, "valid_girth_units", 'report[detail_bodypart_test]['.$rdbtid.'][rdbtrtmname]', $array['rdbtrtmname'], 8, 20, $options['rdbtrtmname']);
...
$cell[]=getDropDownHTML($tableid, $rowid, "rdbtresult1[right]", NULL, NULL, 'report[detail_bodypart_test][add][rdbtresult1][right]['.$tableid.']', NULL, 8, 20, NULL);
if($bilateral) {
	$cell[]=getDropDownHTML($tableid, $rowid, "rdbtresult1[left]", NULL, NULL, 'report[detail_bodypart_test][add][rdbtresult1][left]['.$tableid.']', NULL, 8, 20, NULL);
}
// Comparison ADD values
if($requirecompreportdate) {
	$cell[]=getDropDownHTML($tableid, $rowid, "rdbtresult2[right]", NULL, NULL, 'report[detail_bodypart_test][add][rdbtresult2][right]['.$tableid.']', NULL, 8, 20, NULL);
	if($bilateral) {
		$cell[]=getDropDownHTML($tableid, $rowid, "rdbtresult2[left]", NULL, NULL, 'report[detail_bodypart_test][add][rdbtresult2][left]['.$tableid.']', NULL, 8, 20, NULL);
	}
}
$cell[]=getDropDownHTML($tableid, $rowid, "rdbtrtmname", NULL, NULL, 'report[detail_bodypart_test][add][rdbtrtmname]', NULL, 8, 20, $options['rdbtrtmname']);

//$cell[]=getDropDownHTML($tableid, $rowid, "rdbtrtmname", NULL, NULL, 'report[detail_bodypart_test][add][rdbtrtmname]', NULL, 8, 20, $options['rdbtresult1[rdbtrtmname]']);
