...
$TEST_GROUP='DERMATOMES';
$test_group='dermatomes';
$test_group_title="Dermatomes Title";
...
$options['m1']=getSelectOptions($arrayofarrayitems=$valid_dermatomes_test_measure, $optionvaluefield='value', $arrayofoptionfields=array('value'=>''), $defaultoption=NULL, $addblankoption=FALSE, $arraykey=NULL, $arrayofmatchvalues=array());
...
$cell[]=getDropDownHTML($tableid, $rowid, "rdbtresult1[m1][right]", NULL, NULL, 'report[detail_bodypart_test]['.$rdbtid.'][rdbtresult1][m1][right]', $array['rdbtresult1']['m1']['right'], 8, 20, $options['m1']);

if($bilateral) {
	$cell[]=getDropDownHTML($tableid, $rowid, "rdbtresult1[m1][left]", NULL, NULL, 'report[detail_bodypart_test]['.$rdbtid.'][rdbtresult1][m1][left]', $array['rdbtresult1']['m1']['left'], 8, 20, $options['m1']);
}

// Comparison Values from database
if($requirecompreportdate) {
	$cell[]=getDropDownHTML($tableid, $rowid, "rdbtresult2[m1][right]", NULL, NULL, 'report[detail_bodypart_test]['.$rdbtid.'][rdbtresult2][m1][right]', $array['rdbtresult2']['m1']['right'], 8, 20, $options['m1']);
	if($bilateral) {
		$cell[]=getDropDownHTML($tableid, $rowid, "rdbtresult2[m1][left]", NULL, NULL, 'report[detail_bodypart_test]['.$rdbtid.'][rdbtresult2][m2][left]', $array['rdbtresult2']['m2']['left'], 8, 20, $options['m1']);
	}
}
