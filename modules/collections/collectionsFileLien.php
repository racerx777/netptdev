...
// Format message fields and use notes system to insert note
	...
	$caid=$appid;
	$note=strtoupper("Request to file lien Date:$date Charges:$charges Payments:$payments Lien Amount:$lienamount $note");
	...
	$query = "
		...
		SET calienstatus='RL', calienamount='$lienamount', caliendate='$datetime', calienuser='$lienuser'
		WHERE caid='$caid'
	";
  ...
  <form method="post" name="noteEditForm">
    <fieldset style="text-align:center;">
      ...
      <table cellpadding="5" cellspacing="0">
        <tr>
          <th colspan="3"><?php echo $bnum; ?> Request to File Lien</th>
        </tr>
		<tr>
			<td> Lien Request Date </td>
			<td nowrap="nowrap" style="text-decoration:none"><input id="liendate" name="liendate" type="text" size="10" maxlength="10" value="<?php echo $liendate; ?>" onchange="checkinput(); validateDate(this.id);">
				<img  align="absmiddle" name="anchor1" id="anchor1" src="/img/calendar.gif" onclick="cal.select(document.noteEditForm.liendate,'anchor1','MM/dd/yyyy'); return false;" /></td>
		</tr>
        <tr>
          <td> Charges </td>
        ...
        <tr>
          ...
          <td colspan="2"><input style="text-align:right" name="lienamount" type="text" value="<?php echo($lienamount); ?>"  /></td>
			...
			<tr>
				<td colspan="2" nowrap="nowrap">Additional Lien Filing Notes:</td>
			</tr>
			<tr>
				<td>&nbsp;</td>
        ...
        <tr>
          <td colspan="3"><input id="submitbutton" name="submitbutton" type="submit" value="Confirm request to file <?php echo($lienamount); ?> lien" />
		...
		<tr>
			<td colspan="3"><?php $likenotebutton="Lien"; require_once($_SERVER['DOCUMENT_ROOT'] . '/modules/collections/collectionsNotesList.php'); ?></td>
		</tr>
      </table>
    </fieldset>
  </form>
...
<?php
				...
				error("999","Error of fetch.<br>$query<br>".mysqli_error($dbhandle));
// error on fetch
			}
		}
		else {
// error not == 1 row
			error("998","Error numrows $numrows.<br>$query<br>".mysqli_error($dbhandle));
		}
	}
	else {
// error result
		error("997","Error result.<br>$query<br>".mysqli_error($dbhandle));
	}
displaysitemessages();
}
?>
