...
document.title="Collection Print Confirmation Letter"
...
function checkinput() {
	...
	var vsubmitbutton = document.getElementById("CreateConfirmationLetter");
...
if(isset($_POST['CreateConfirmationLetter'])) {
// Format message fields and use notes system to insert note
	...
	$note="Confirmation Letter Sent Date:$date Amount:$amount Via:$sendvia Adj:$insadjuster Ins:$insname $insaddress $inscity $insstate $inszip";
	...
	if(
		empty($_POST['casettlestatus']) ||
		$_POST['casettlestatus']=='NON' ||
		$_POST['casettlestatus']==NULL ||
		$_POST['casettlestatus']=='??' ||
		$_POST['casettlestatus']=='DLS' ||
		$_POST['casettlestatus']=='CLS') {
	require_once($_SERVER['DOCUMENT_ROOT'] . '/modules/collections/collectionsSettleStatusFunctions.php');
	collectionsSettleStatusUpdate($appid, 'CLS', $date, $amount);
	}
...
foreach($_POST as $key=>$val) {
	...
	echo("<script>");
	echo("window.open('/modules/collections/collectionsPrintForms.php?$req','CreateConfirmationLetter');");
...
// Actions should be taken here. Default to PTOS if collections account never updated
...
<div class="centerFieldset">
  <form method="post" name="noteEditForm">
    <fieldset style="text-align:center;">
      <legend> Send Confirmation Letter for <?php echo "$bnum $pnum - $patientname" ?></legend>
      <table cellpadding="5" cellspacing="0">
        <tr>
          <td nowrap="nowrap">Confirmation Date and Amount</td>
        ...
        <tr>
          <td colspan="2"><input name="CreateConfirmationLetter" id="CreateConfirmationLetter" type="submit" value="Create Confirmation Letter"  /></td>
