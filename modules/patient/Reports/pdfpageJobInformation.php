...
function pdfJobInformation($p, $settings, $data, $lang='en') {
	...
	$pdfJobInformationTitle['en']="Job Information";
	$pdfJobInformationTitle['sp']="Informacion del trabajo";
	
	$bodyleft=32;
	$bodytop=710;

	pdf_begin_page($p, 612, 792);
	pdf_putheader($p, 32, 700, $pdfJobInformationTitle["$lang"], $case['clinic'], $lang);
	$newbodytop = pdf_putpatientinformationshort($p, $bodyleft, $bodytop, $data, $lang); //	Patient Information
	$newbodytop = pdf_putjobinformation($p, $bodyleft, $newbodytop-(12*(-6.5)), $data, $lang);//Current Job Information
	pdf_end_page($p);
}
?>