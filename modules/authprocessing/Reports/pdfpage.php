...
$w=576;
$h=300;
pdf_puttext($p, $x, $y, $text1);
pdf_puttextboxed($p, 150, $y-16, $w, $h, $text);
return($y-16-(6*16));
}

function pdf_putstandardletterheader($p, $x, $y, $data, $addressarraykey=NULL) {
	if(!empty($data['today']))
		$date=$data['today'];
	else
		$date=today();
	$date=displayDate($date, "m/d/Y");

	if(!empty($addressarraykey))
		$address=$data["$addressarraykey"];
	else
		$address=$data["address"];
	$line=array();
	$line[]="Date: $date";
	if(!empty($address['name']))
		$line[]=strtoupper($address['name']);
	if(!empty($address['address1']))
		$line[]=strtoupper($address['address1']);
	if(!empty($address['address2']))
		$line[]=strtoupper($address['address2']);
	if(!empty($address['city']))
		$city=strtoupper($address['city']);
	if(!empty($address['state']))
		$state=strtoupper($address['state']);
	if(!empty($address['zip']))
		$zip=displayZip($address['zip']);
...
$line7
";
pdf_puttextboxed($p, 54+18, 585+18+(4*12), 324-18-18, 90-18-18, $text);
}
...
$text2="Accepted and agreed upon by (Sign and Date)


_______________________ _________
".$data['adjusterfname']." ".$data['adjusterlname']; 
$w=576;
$h=300;
pdf_puttextboxed($p, $x, $y, $w, $h, $text);
pdf_puttextboxed($p, $x, $y-32, $w, $h, $text1);
pdf_puttextboxed($p, $x+300, $y-32, $w, $h, $text2);
return($y-(10*16));
}
