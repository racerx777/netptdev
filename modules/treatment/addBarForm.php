...
?>
		...
		<div style="clear:both; margin:10px;">
			...
			<div style="float:right">
				<?php
				if(isset($_SESSION['useraccess']['clinics']) && empty($_SESSION['useraccess']['clinics']))
				{
   					$disabled="disabled";
   					?>
					<script>
						alert('Your clinic is currently at an inactive status. You are able to search for prior treatments but not add new treatments.');
						$('#search-treatments').trigger("click");
					</script>
					<?php
   				}
   				else
				...
				foreach($clinics as $pause){
					...
					if( $get['pausestate'] == 0 ) {
						if ($count == 1) {
							?>
							<input id="actionbutton" name="button[]" type="submit" value="Add" <?php echo $disabled; ?> onclick="javascript:onsubmitfunction();" />
							<?php
							$count++;
						}
					}
