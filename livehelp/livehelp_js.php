...
{ 
  ...
  ?>
  //alert(html);
  createLayer('mylayer<?php echo intval($UNTRUSTED['department']); ?>Div',null,-500,-500,550,400,html,null,null,999)
	mylayer<?php echo intval($UNTRUSTED['department']); ?> = new aDynLayer('mylayer<?php echo intval($UNTRUSTED['department']); ?>Div');
	 ...
	 popcontrol2.src = u;	
	 stillopen = 1;
	 mylayer<?php echo intval($UNTRUSTED['department']); ?>.moveTo(-200,-200);
...
{ 
  if(stillopen==1){
   ...
   gox = mylayer<?php echo intval($UNTRUSTED['department']); ?>.x;
   goy = mylayer<?php echo intval($UNTRUSTED['department']); ?>.y;
   ...
   mylayer<?php echo intval($UNTRUSTED['department']); ?>.moveTo(gox,goy);
   if(goy!= myWidth+160) 
      setTimeout('moveDHTML_<?php echo intval($UNTRUSTED['department']); ?>()', 2);
   else
