...
if($UNTRUSTED['action'] == "addnew"){
 ...
 $query = "INSERT INTO livehelp_users (username,password,isoperator,isadmin,isnamed,email,user_alert,show_arrival,identity,greeting,showtype) 
           VALUES ('".filter_sql($UNTRUSTED['newuser'])."','".filter_sql(md5($UNTRUSTED['newpass']))."','Y','".filter_sql($UNTRUSTED['isadmin'])."','Y','".filter_sql($UNTRUSTED['email'])."','".filter_sql($UNTRUSTED['user_alert'])."','".filter_sql($UNTRUSTED['show_arrival'])."','$md5string','".filter_sql($UNTRUSTED['greeting'])."','".filter_sql($UNTRUSTED['showtyping'])."')";	
