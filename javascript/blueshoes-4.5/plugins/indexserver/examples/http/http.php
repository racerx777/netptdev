...
if ((!@$_POST['step'] !== '2') && $doIndex) {
	...
	$status = $is->dropIndex('filesystem'); //ignore if it did not exist
	$status = $is->createIndex('filesystem', $xmlStr='');
	
	$indexer  = &$is->getIndexer('filesystem');
	if (isEx($indexer)) {
...
$searcher = &$is->getSearcher('filesystem');
