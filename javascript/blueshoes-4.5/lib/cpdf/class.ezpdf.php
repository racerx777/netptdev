...
* @access public
* @param  string $hex (like 'FFFFFF' or '#ffffff'.)
* @return array (vector with 3 elements: 0=red, 1=green, 2=blue.)
...
*/
function invertY($y) {
	return $this->ez['pageHeight'] - $y;
}

/**
* Renders the given list to the pdf.
* Something like the <ul>, <ol> and <li> tags of html.
* 
* data structure for param $list:
* 	it is an array, usually a vector (with numeric keys). the keys are basically ignored.
* 	example: 
* 	$list = array(
* 		'element one', 
* 		'element two', 
* 	);
* 	an element can have children. this can be done as vector or as hash (string keys).
* 	example: 
* 	$list = array(
* 		'element one', 
* 		'element two', 
* 		'element three' = array(
* 			'first child of a hash', 
* 			'second child of a hash', 
* 		), 
* 		'element four', 
* 		array(
* 			'first child of a vector', 
* 			'second child of a vector', 
* 		), 
* 	);
* 	the difference between the hash and the vector is that the key of a hash is used as 
*   element aswell. check the example file if you don't get it yet.
* 
* data structure for param $props:
* 	it is a hash with 0-all of the keys:
* 		KEY									DEFAULT VALUE			DESCRIPTION
* 		fontSize						10								obvious.
* 		lineHeight					15								minimal line heigh; the actual line height may be higher.
* 		indentLeftBullets		50								how much to indent from the left side for bullets. in recursive 
* 																					calls (children) that number means "additional indent".
* 		indentLeftTextDiff	15								how much to indent from the bullet start to the text start.
*     indentRight         NULL              if specified then this will passed to as 'right' to ezText(). 
*                                           it is the gap on the right side of the page. for details see ezText().
* 		indentTopBullets  	y 								how much to indent from the top. if not specified then the current 
* 																					y position ($this->y) will be used. note that this var is not always 
* 																					up to date! but you can set it before calling ezList().
* 		indentTopTextDiff  	0 								if the text should have another indent from the top than the bullets, 
* 																					then specify this here. positive numbers = move down, negative = move up.
* 																					this is needed to get the bullet and text on a lign, especially when using 
* 																					different bullet types/sizes, fonts etc.
* 		bulletType  				'square'					options are: 
* 																					square   => a filled square.
* 																					circle   => a filled circle. name equivalent to html. see the todo list.
* 																					disc     => a non-filled circle. name equivalent to html.
* 																					alpha    => characters a-z or A-Z
* 																					numarab  => numbers 1-n
* 		bulletProps  				[depends]					an array with more settings that are different based on 'bulletType'.
*       all-of-them:												
* 				color						[none]						a color for the bullet.
* 			circle and disc: 										
...
* 			numarab: 														
* 				fontSize				[depends]					font size for the number. falls back to the font size of the text.
* 				separator				')'								so the number will look like 1), 2)... it could be a dot or whatever.
* 				start						1									at which number to start.
* 			alpha: 															
* 				fontSize				[depends]					font size for the number. falls back to the font size of the text.
* 				separator				')'								so the character will look like a), b)... it could be a dot or whatever.
* 				start						'a'								at which letter to start. after z there comes aa, then ab, and so on.
* 																					you can also use uppercase A etc.
* 
* to be more flexible with nested arrays and properties, the $list param can be a hash with the keys 
* 'values' and 'props', and thus look like:
* $list = array(
* 	'values'  => array(
* 		//here comes the usual data array, and data arrays again can be hashes with the 
* 		//keys 'values' and 'props'. check the example file.
*		), 
* 	'props'  => array(
* 		//here comes the usual properties array
*		), 
* );
* if there is a 'props' key then the 2nd param ($props) in the ezList() call will be ignored. this rule 
* also applies for subarrays (children).
* 
* todo: 
*   - add bulletType: dash
*   - make bulletType 'circle' usable. i was not able to fill the circle, and that's the only 
*     difference to 'disc'. i tried to draw many small circles in it, but it's not filled completely.
* 	- add the $test param like in ezText() using transactions. that's fun. currently if the list is too long, 
*     it draws over the border at the bottom of the page. this does not happen on the right side thanks to ezText().
*   - add bulletType: image (to use images as bullets). just in case you're bored. *g*
* 
* 
* @access public
* @param  array $list  (see above.)
* @param  array $props (hash, default is an empty array, see above.)
* @return int (the new y position.)
* @author andrej at blueshoes dot org
*/
function ezList($list, $props=array()) {
	
	//what's the structure of the given array?
	if (isSet($list['props'])) {
		$props = $list['props'];
	}
	...
	//merge default props with given props:
	$propsDefault = array(
		'fontSize'           => 10, 
		'lineHeight'         => 15, 
		'indentLeftBullets'  => 50, 
		'indentLeftTextDiff' => 15, 
		'indentRight'        => NULL, 
		'indentTopBullets'   => $this->invertY($this->y), 
		'indentTopTextDiff'  => 0, 
		'bulletType'        => 'square', 
		'bulletProps'       => array(), 
	);
	$props = array_merge($propsDefault, $props);
	
	switch ($props['bulletType']) {
		case 'circle':
			$bulletPropsDefault = array(
				'radius'        => 3, 
			);
			break;
		case 'disc':
		...
		case 'alpha':
			...
			break;
		default: //also case 'square'
			$bulletPropsDefault = array(
				'width'         => 5, 
				'height'        => 5, 
			);
	}
	$props['bulletProps'] = array_merge($bulletPropsDefault, $props['bulletProps']);
	
	$yNow = $props['indentTopBullets'];
	if (empty($list) || !is_array($list)) return $yNow;
	
	$liCount = 0;
	foreach ($list as $key => $li) {
		$yNow += 10;  //+10: somehow the text starts too much down the page. strange, dunno why. we put the ellipse 
		...
		if (!is_null($liTxt)) {
			...
			switch ($props['bulletType']) {
				case 'circle':
				case 'disc':
					if (!empty($props['bulletProps']['color'])) {
						$lastColor = $this->currentStrokeColour; //remember the last setting
						$t = $this->hexToRgb($props['bulletProps']['color']);
						$this->setStrokeColorBy255($t[0], $t[1], $t[2]);
					}
					break;
				case 'numarab':
				case 'alpha':
				default: //also case 'square'
					if (!empty($props['bulletProps']['color'])) {
						$lastColor = $this->currentColour; //remember the last setting
						$t = $this->hexToRgb($props['bulletProps']['color']);
						$this->setColorBy255($t[0], $t[1], $t[2]);
					}
			}
			...
			switch ($props['bulletType']) {
				case 'circle':
				case 'disc':
					if ($props['bulletType'] === 'circle') {
						for ($i=$props['bulletProps']['radius']; $i>0; $i--) {
							$this->ellipse($props['indentLeftBullets'], $this->invertY($yNow - ($i /2)), $i);
						}
					} else {
						$this->ellipse($props['indentLeftBullets'], $this->invertY($yNow - ($props['bulletProps']['radius'] /2)), $props['bulletProps']['radius']);
					}
					break;
				case 'numarab':
					$this->addText($props['indentLeftBullets'], $this->invertY($yNow), $props['bulletProps']['fontSize'], ($props['bulletProps']['start'] + $liCount) . $props['bulletProps']['separator']);
					break;
				case 'alpha':
					$char = '';
					//find diff to a or A:
					if (strToLower($props['bulletProps']['start']) === $props['bulletProps']['start']) { //it's a lowercase char
						$base = 97;
						$diff = ord($props['bulletProps']['start']) - 97;
					} else { //uppercase char
						$base = 65;
						$diff = ord($props['bulletProps']['start']) - 65;
					}
					$x1 = floor(($liCount + $diff) / 26);
					$x2 = ($liCount + $diff) % 26;
					if ($x1 > 0) {
						$char .= chr($x1 + $base -1);
					}
					//$charNum = ord($props['bulletProps']['start']) + $liCount;
					$char .= chr($x2 + $base);
					$this->addText($props['indentLeftBullets'], $this->invertY($yNow), $props['bulletProps']['fontSize'], $char . $props['bulletProps']['separator']);
					break;
				default: //also case 'square'
					$this->filledRectangle($props['indentLeftBullets'], $this->invertY($yNow), $props['bulletProps']['width'], $props['bulletProps']['height']);
			}
			...
			switch ($props['bulletType']) {
				...
				case 'disc':
					if (!empty($props['bulletProps']['color'])) {
						//set orig color back:
						$this->setStrokeColor($lastColor['r'], $lastColor['g'], $lastColor['b']);
					}
					break;
				case 'numarab':
				case 'alpha':
				default: //also case 'square'
					if (!empty($props['bulletProps']['color'])) {
						//set orig color back:
						$this->setColor($lastColor['r'], $lastColor['g'], $lastColor['b']);
					}
			}
		...
		if (!empty($props['fontColor'])) {
			$lastColor = $this->currentColour; //remember the last setting
			$t = $this->hexToRgb($props['fontColor']);
			$this->setColorBy255($t[0], $t[1], $t[2]);
		}
		
		
		if (!is_null($liTxt)) {
			$this->y = $this->invertY($yNow + $props['indentTopTextDiff']);
			$yBackup = $yNow;
			
			$ezTextProps = array('aleft'=>$props['indentLeftBullets'] + $props['indentLeftTextDiff']);
			if (is_numeric($props['indentRight'])) $ezTextProps['right'] = $props['indentRight'];
			$yNow = $this->invertY($this->ezText($liTxt, $props['fontSize'], $ezTextProps));
			if (!empty($props['fontColor'])) {
				//set orig color back:
				$this->setColor($lastColor['r'], $lastColor['g'], $lastColor['b']);
			}
			if ($yNow - $yBackup < $props['lineHeight']) {
				$yNow += ($props['lineHeight'] - ($yNow - $yBackup));
			}
		...
		if (is_array($li)) {
			if (isSet($li['props'])) {
				//props ship with data
				$li['props']['indentLeftBullets'] = $props['indentLeftBullets'] + $props['indentLeftTextDiff'] + @$li['props']['indentLeftBullets'];
				$yNow = $this->ezList($li);
			} else {
				//need to pass along some props
				$newProps = $props;
				$newProps['indentLeftBullets'] = $newProps['indentLeftBullets'] + $newProps['indentLeftTextDiff'] + 5;
				$newProps['indentTopBullets']  = $yNow;
				$yNow = $this->ezList($li, $newProps);
			}
		}
