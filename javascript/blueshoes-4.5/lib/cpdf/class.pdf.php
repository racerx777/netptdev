...
* @param  int $force (0 or 1, like in setColor().)
* @return void
* @author andrej at blueshoes dot org
*/
function setColorBy255($r, $g, $b, $force=0) {
	$r /= 255;
	$g /= 255;
	$b /= 255;
	$this->setColor($r,$g,$b,$force);
}
...
* @param  int $g (green color value, 0-255.)
* @param  int $b (blue  color value, 0-255.)
* @param  int $force (0 or 1, like in setStrokeColor().)
* @return void
* @author andrej at blueshoes dot org
*/
function setStrokeColorBy255($r, $g, $b, $force=0) {
	$r /= 255;
	$g /= 255;
	$b /= 255;
	$this->setStrokeColor($r,$g,$b,$force);
}
...
/**
* @access public
* @param  int    $x    (x-axis, text will go to the left from there)
* @param  int    $y    (y-axis)
...
* @author andrej at blueshoes dot org
*/
function addTextAlignRight($x, $y, $size, $text) {
	$textWidth = $this->getTextWidth($size, $text);
	$this->addTextWrap($x -$textWidth, $y, $textWidth, $size, $text, 'right');
}
...
function addTextWrap($x,$y,$width,$size,$text,$justification='left',$angle=0,$test=0){
  ...
  /*
	...
	*/
	if ($width <= 0) {
		$width = $this->ez['pageWidth'] - $x - $this->ez['rightMargin'];
		if ($x == 0) $width -= $this->ez['leftMargin'];
	}
