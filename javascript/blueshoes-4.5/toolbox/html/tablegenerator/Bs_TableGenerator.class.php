...
*/?><?php
if (!isSet($GLOBALS['APP']['path']['diffFromDocRootToGlobalConf'])) $GLOBALS['APP']['path']['diffFromDocRootToGlobalConf'] = '../';require_once($_SERVER['DOCUMENT_ROOT'] . $GLOBALS['APP']['path']['diffFromDocRootToGlobalConf'] . 'global.conf.php');require_once($APP['path']['core'] . 'util/Bs_StopWatch.class.php');require_once($APP['path']['core'] . 'file/Bs_Dir.class.php');require_once($APP['path']['core'] . 'html/Bs_HtmlUtil.class.php');require_once($APP['path']['core'] . 'html/table/Bs_HtmlTable.class.php');require_once($APP['path']['core'] . 'html/table/Bs_HtmlTableWindrose.class.php');class Bs_TableGenerator extends Bs_Object {var $_sampleMatrix = array(array('NW', 'N',  'N', 'NE'),
array( 'W', 'C',  'C', 'E' ),
array( 'W', 'C',  'C', 'E' ),
array('SW', 'S',  'S', 'SE'));function Bs_TableGenerator() {$this->Bs_Object();$this->_Bs_HtmlUtil = &$GLOBALS['Bs_HtmlUtil']; }
...
function  &_toPhpCode(&$htmlWindroseObj) {$styleArray = &$htmlWindroseObj->getStyleStrings();$attrArray = &$htmlWindroseObj->getAttrStrings();$phpCode = '    <?'."\n";$phpCode .= <<< EOD
      require_once('blueshoes.ini.php');
...
function &getEditHtml($input=NULL) {global $APP;$info = <<< EOD
    ...
    <br><br>
    <strong>NOTE:</strong> <br>&nbsp;Use the right syntax for syles and attributes. E.g.:<br>
    &nbsp;&nbsp;&nbsp;&nbsp;Style: color:blue; background-color:lightgrey; font-family:arial; <br>
    &nbsp;&nbsp;&nbsp;&nbsp;Attr: align=right  background=compassBg.gif
    </SPAN>
EOD;
$input['todo'] = isSet($input['todo']) ? $input['todo'] : '';$phpCodeSample = '';if (isSet($input)) {$htmlWindroseObj =& new Bs_HtmlTableWindrose();if (($input['todo']==='readStyle') AND isSet($input['stylefile'])) {$styleFile = $APP['path']['bsRoot'] . 'toolbox/Html/TableGenerator/styles/' . $input['stylefile'];$htmlWindroseObj->read($styleFile.'.style');}
...
$sampleTbl =& new Bs_HtmlTable($this->_sampleMatrix);$sampleTbl->setWindroseStyle(&$htmlWindroseObj);$sampleTbl->setTableAttr(array('cellspacing'=>'0', 'cellpadding'=>'0'));$saveStyleName = isSet($input['stylefile']) ? $input['stylefile'] : 'default';$data = $this->_matrix(3, 2);$data[0][0] = $this->_getMnomoicPictogram();$data[1][0] = $sampleTbl;$data[1][1] = 'Save windrose style and attr. to file:<br><input type="text" name="input[stylefile]" size="35" value="'.$saveStyleName.'"><BR><input type="submit" name="input[submit]" value="save style"><br><br>';$data[2][0] = $this->_getFormInputTable($htmlWindroseObj);$tbl =& new Bs_HtmlTable($data);$tbl->setRowAttr(1,array('align'=>'center'));$tbl->spanRow(0);$tbl->spanRow(2);$tblString = &$tbl->renderTable();global $PHP_SELF;$action = $PHP_SELF;$out = <<< EOD
    {$info}
    <CENTER>
...
EOD;
return $out;}
function &getStyles() {global $PHP_SELF;$action = $PHP_SELF;$out  = "<CENTER><form action='{$action}' method='post' target='edit'> \n";$out .= '<input type="hidden" name="input[todo]" value="readStyle">' . "\n";$htmlWindroseObj =& new Bs_HtmlTableWindrose();global $APP;$aDir =& new Bs_Dir($APP['path']['bsRoot'] . 'toolbox/Html/TableGenerator/styles/');$fileList = $aDir->getFileList(array('regEx'=>'\.style$'));$tbl =& new Bs_HtmlTable($this->_sampleMatrix);$tbl->setTableAttr(array('cellspacing'=>'0', 'cellpadding'=>'0'));for ($i=0; $i<sizeOf($fileList); $i++) {$htmlWindroseObj->read($fileList[$i]);$tbl->setWindroseStyle($htmlWindroseObj);$out .= $tbl->renderTable();$fileName = basename($fileList[$i]);$fileName = substr($fileName, 0, strpos($fileName, '.'));$out .= '<input type="submit" name="input[stylefile]" value="'.$fileName.'">';$out .= '<br><br>';}
$out .= '</form></CENTER>';return $out;}
function &_matrix($row, $col, $value='&nbsp;') {$matrix = array();for ($i=0; $i<$row; $i++) {for ($j=0; $j<$col; $j++) {$matrix[$i][$j] = $value;}
}
return $matrix;}
}  
if (!isSet($input)) {$html = <<< EOD
    <html>
      <frameset cols="80%,*">
        <frame src="{$PHP_SELF}?input[target]=edit" name="edit">
        <frame src="{$PHP_SELF}?input[target]=stylelist" name="stylelist">
      </frameset>
    </html>
EOD;
echo $html;exit;} 
$stopWatch = $GLOBALS['Bs_StopWatch'];$stopWatch->reset();$tbl =& new Bs_TableGenerator;if (isSet($input['target']) AND ($input['target']==='stylelist')) {$out = &$tbl->getStyles();} else {$out = &$tbl->getEditHtml($input);}
