...
function icd9CodeOptions($includeinactive=0, $desccase=NULL) {
	...
	if(empty($includeinactive))
		$query = "SELECT imicd9, imdx FROM master_ICD9 WHERE iminactive=0 ORDER BY imicd9 ";
	else
		$query = "SELECT imicd9, imdx FROM master_ICD9 ORDER BY imicd9 ";
	if($result = mysqli_query($dbhandle,$query)) {
		while($row = mysqli_fetch_assoc($result)) {
			$code=$row['imicd9'];
...
endswitch;
        ...
        if ($_SESSION['user']['umuser'] == 'GeoffSundstrom'
